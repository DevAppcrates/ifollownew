...
class ContactCenterController extends Controller
{
    ...
    public function notification(Request $request)
    {
        ...
        if ($request->schedule == 0) {
            ...
            $invited_group = Groups::where('title', 'Invited Users')->whereIn('id', $groups)->first();
            if ($invited_group) {
                $invitees = Invitees::where('organization_id', $invited_group['organization_id'])->get();
                if (count($invitees)) {
                    $emails = $invitees->pluck('email')->toArray();
                    $phone_numbers = $invitees->pluck('phone')->toArray();
                    Mail::send([], [], function ($message) use ($emails, $notification) {
                        $body = $notification->priority . ' Alert<br>';
                        ...
                        $message->bcc($emails)->subject($notification->priority . ' Alert ' . 'Ifollow | Mass Notification(MN) - ' . session('contact_center_admin.0.organization_name'))->setBody($body, 'text/html');
                    });
                    $accountSid = 'AC7966ed796f64aa074e05e7db1d982a36';
                    $authToken = '002a6ef044af01d69cb979eb4107b9fe';
                    $twilioNumber = "+14359195249";
                    $body = $notification->priority . ' Alert ' . "Ifollow | Mass Notification(MN) - " . session('contact_center_admin.0.organization_name') . "\n\n" . $notification->title . "\n\n";
                    // $body .= $notification->priority.' Alert'."\n\n";
                    if ($notification->notification) {
                        $body .= $notification->notification;

                    }
                    foreach ($phone_numbers as $numberitem) {
            ...
            if (count($users)) {
                ...
                foreach ($users as $value) {
                    $phone_numbers[] = $value['country_code'] . '' . $value['phone_number'];
                }
                ...
                $fields = array(
                    'notification' => array(
                        'title' => $notification->priority . ' Alert' . "\n\n" . $request->title,
            ...
            if (count($invitees_users)) {
                $emails = $invitees_users->pluck('email')->toArray();
                $phone_numbers = $invitees_users->pluck('phone')->toArray();
        ...
        if ($request->schedule == 0) {
            ...
            if ($invited_group) {
                ...
                if (count($invitees)) {
                    ...
                    Mail::send([], [], function ($message) use ($emails, $notification) {
                        $body = $notification->priority . ' Alert<br>';
                        $body .= '<strong>' . $notification->title . '</strong>';
                        if ($notification->notification) {
                            $body .= '<br>' . $notification->notification;
                        }


                        $message->bcc($emails)->subject('Ifollow | Mass Notification(MN) - ' . session('contact_center_admin.0.organization_name'))->setBody($body, 'text/html');
                    });
                    $accountSid = 'AC7966ed796f64aa074e05e7db1d982a36';
                    $authToken = '002a6ef044af01d69cb979eb4107b9fe';
                    $twilioNumber = "+14359195249";
                    $body = "Ifollow | Mass Notification(MN) - " . session('contact_center_admin.0.organization_name') . "\n\n" . $notification->title . "\n\n";
                    $body .= $notification->priority . ' Alert' . "\n\n";
                    if ($notification->notification) {
                        $body .= $notification->notification;
            ...
            if (count($users)) {
                ...
                $authToken = '002a6ef044af01d69cb979eb4107b9fe';
                $twilioNumber = "+14359195249";
                $body = "Ifollow | Mass Notification(MN)" . "\n\n" . $notification->title . "\n\n";
                if ($notification->notification) {
                    $body .= $notification->notification;
            ...
            if ($request->hasFile('csv')) {
                ...
                foreach ($results as $result) {
                    if (!Invitees::where('organization_id', $organization_id)->where('email', $result['email'])->first()) {
                        ...
                        $this->send_message($result['email'], $result['phone'], $message);
                    }
    ...
    public function single_invitees(Request $request)
    {
        ...
        try {
            ...
            $this->send_message($request->email, $invite->phone, $message);
    ...
    private function send_message($email, $phone, $msg)
    {
        try {
            Mail::send([], [], function ($message) use ($msg, $email) {
                $message->to($email)->subject('iFollow | Invites')->setBody($msg, 'text/html');
            });
        } catch (\Exception $e) {
        }
    ...
    public function notification_user_status($user_id, $notification_id, $type)
    {
        ...
        return View('contact_center.message');
    }
}
