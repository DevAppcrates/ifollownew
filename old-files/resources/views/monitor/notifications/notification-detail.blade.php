@extends('monitor.layout.default')
@section('title')
{{ config('app.name') }} | Notification Details
@stop
@section('css')
...
<div class="page-content-wrapper">
   ...
   <div class="page-content">
      ...
      <div class="row data-view" style="margin-top: 3%">
         <div class="col-md-7">

            <div class="row">
               <div class="<?php echo $notification_detail->type == 0 ? 'col-lg-12 col-md-12 col-sm-12' : 'col-lg-6 col-md-6 col-sm-12'; ?>">
                  <h3 class="col-heading">Notification Title:</h3>
                     <span class="sub-heading">{{$notification_detail->title}}</span>
                     <p class="message-box" @if($notification_detail->type == 0) @endif>
                        {{ $notification_detail->notification?$notification_detail->notification:'N/A' }}
                     </p>
               </div>
               <div class="col-lg-6 col-md-6 col-sm-12">
                  @if($notification_detail->type != 0)
                        <div class="col-sm-12 pull-left" style="margin-top: 45px;">
                           @if($notification_detail->type==2)
                                 <b>Video:</b>
                                    <video style="width: 320px;height: 420px;object-fit: contain;" width="180" height="200" class="img-thumbnail img-responsive" controlsList="nodownload" controls >
                                       <source src="{{$notification_detail->path}}" type="video/mp4">
                                       <source src="{{$notification_detail->path}}" type="video/ogg">
                                          Your browser does not support the video tag.
                                    </video>
                           @elseif($notification_detail->type==3)
                                 <b>Audio:</b>
                                    <audio style="width: 100%;" controls controlsList="nodownload">
                                       <source src="{{ $notification_detail->path }}" type="audio/wav">
            ...
            <br>
            <div class="intiated-by">
               <h3 class="col-heading">Initiated By:</h3>
               <span class="sub-heading">{!! $notification_detail['sent_by']['name']?$notification_detail['sent_by']['name']:$notification_detail['name'] !!}</span>
               <br>
               <div class="tag-box">
                  ...
                  <p class="lead">
                     ...
                     {{ $notification_detail->priority?$notification_detail->priority .'':"N/A"  }}
                  </p>
               </div>
               <div class="tag-box">
                  <label><b># Of Groups Transmitted To:</b></label>
                  <p class="lead"><i class="fa fa-users"></i> {{ $notification_detail['groups_count'].' Group(s)'  }}</p>
               </div>
               <div class="tag-box">
                  <label><b># Of Users Transmitted To:</b></label>
                  <p class="lead"><i class="fa fa-users"></i> {{ count($user_ids).' Users(s)'  }}</p>
               </div>
               <br>
               <div class="tag-box">
                  <label><b>Date/Time Created:</b></label>
                  <p class="lead"><i class="fa fa-clock-o"></i>{{ $notification_detail['created_at']  }}</p>
               </div>
               <div class="tag-box">
                  <label><b>Mass Notification(MN) Type:</b></label>
                  <p class="lead"><i class="fa fa-circle"></i>
               ...
               <div class="tag-box">
                  ...
                  <p class="lead">
                      {!! $notification_detail['status'] == "Sent"?"<i class='fa fa-check-circle'></i>" :"<i class='fa fa-minus-circle'></i>" !!} {{ $notification_detail['status']  }}
                  </p>
               </div>
               <div class="tag-box">
                  ...
                  <p class="lead">
                     @if($notification_detail->is_archive)
                        <i class="fa fa-archive"></i> {{ 'Yes'  }}
                     @else
                        <i class="fa fa-archive"></i> {{ 'No'  }}
                     @endif
                  </p>
               </div>
            </div>
         ...
         <div class="col-md-5 list-table-result">
            <h3 class="col-heading">Group List</h3>
            <br>
            <div class="portlet-body responsive">
              <table class="table-striped table-bordered table-success responsive table-condensed"  id="example2">
                           ...
                           <tbody>
                              ...
                              <tr>
                                 ...
                                 <td >{{$group->title}}</td>
                                 <td ><a data-id='{{ $group->id }}' style="padding-top: 10px;" class="btn btn-sm btn-outline blue" onclick="view_members('{{ $group->id }}',this)">
             ...
             @if(session('monitor.0.organiztions.0.organization.type') != 2)
            <h3 class="col-heading">Manage</h3>
            <br>
            ...
            @elseif(session('monitor.0.organiztions.0.organization.type') == 2 && $notification_detail->status == 'Pending')
               <h3 class="col-heading">Manage</h3>
               <br>
               <div class="list-group">
                  @if($notification_detail->status == 'Pending')
...
<div class="modal fade" id="edit_push" tabindex="-1" role="dialog">
   <div class="modal-dialog " role="document">
      ...
      <div class="modal-content" style="margin-top: 70px">
         ...
         <!--Body-->
            <form id="edit-push" novalidate="novalidate">
                  <div class="modal-body">
                        <div class="form-group" id="form-group">
                           ...
                           <select id="getTemplateDropdown" onchange="getTemplate(this)" class="form-control" name="">
                              ...
                              <?php $templates = \App\Templates::where('organization_id', session('monitor.0.organiztions.0.organization.organization_id'))->where('status', 1)->get();?>
                  ...
                  <br>
                  <audio controls muted id="audio-player-edit-push" style="display: none" controlsList="nodownload"></audio>
                  </section>
                  <audio controls id="edit-hear-audio" style="display: none;">
                  <source src="" id="edit-hear-audio-src" type="audio/wav">
                  </audio>
                  <br/>
                  <div class="form-group" id="form-group">
                  ...
                  <?php $groups = \App\Groups::withCount('members')->where('organization_id', session('monitor.0.organiztions.0.organization.organization_id'))->where('status', 1)->get();?>
                  <?php $invitees = \App\Invitees::where('organization_id', session('monitor.0.organiztions.0.organization.organization_id'))->count();
?>
               ...
               <div class="model-footer">
                  <button class="btn btn-primary waves-effect" id="editnotificationButton" >Update Notification</button>
               </div>
         </form>
      <!--/.Content-->
   </div>
...
<script type="text/javascript">
   ...
   function view_members(group_id,current)
   {
       ...
       if(group_name === 'Invited Users')
       {
          ...
          $.ajax({
           type : 'GET',
           url : "{{ url('contact_center/ajax/get_group_members_with_notification') }}/"+group_id+'/'+'{{$notification_detail->id}}'+'/'+group_name,
       ...
       $.ajax({
           type : 'GET',
           url : "{{ url('contact_center/ajax/get_group_members_with_notification') }}/"+group_id+'/'+'{{$notification_detail->id}}',
           success : function(data,status)
           {
               $("#GroupMembersLabel").html(data.group.title+" Members");
               $.each(data.response,function(key,value){
       ...
       $('#edit-push').validate({
           ...
           submitHandler:function(form){
               ...
               if(schedule_dropdown != 0)
               {
                   ...
                   $.ajax({
                       url:"{{ url('/') }}/contact_center/ajax/edit_notification",
                       type:'post',
                       cache: "false",
                       contentType: false,
                       processData: false,
   ...
   $(document).on('click','.btnDelete',function(){
       ...
       bootbox.confirm("You are going to Delete a Mass Notification. Are you sure?", function (result) {
           if (result == true) {
               ...
               $.ajax({
                   type : "GET",
                   url :"{{ url('/monitor-hub/ajax/delete_notification') }}",
                   ...
                   success : function(data,status)
                   {
                       ...
                       window.setTimeout(function() {
                           ...
                           window.location.href = '{{ url('/monitor-hub/notifications') }}'
                       }, 2000)
                   }
               })
           }
   ...
   $(document).on('click','.btnAddTOArchived',function(){
       ...
       bootbox.confirm("You are going to archive a Mass Notification. Are you sure?", function (result) {
           if (result == true) {
               ...
               $.ajax({
                   type: "POST",
                   url: "{{url('/monitor-hub/ajax/add_archive')}}",
                   data: "notification_id="+notification_id,
                   success : function(data,status){
                       table = $('#example2').DataTable();
