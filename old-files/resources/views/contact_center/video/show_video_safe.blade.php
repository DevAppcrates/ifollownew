...
.video-section{
 ...
 box-shadow: 0px 0px 7px #00000029 !important;
...
<br>
    ...
    <div class="row">
             @if($video->type != 'panic')
            <div class="col-md-3">
                <div style="height: 2px; width: 100% !important: background-color: transparent;">
                </div>
            </div>
            @endif
              <div  @if($video->type !='panic')class="col-md-6"@else class="col-md-7" @endif>
            <h3 class="h3"><img style="float:right;margin-top: -15px;margin-left: -10px;" height="40px" width="40px" src="{{ asset('public/images/live.gif') }}">VIDEO</h3>
                <div class="video-section">
                    <div id="videos">
        <br>
        <br>
            <div id="subscriber" style="height: 330px; margin-top: -40px;"></div>
            <div id="soon" style="display: none">
                <img src="{{url('/')}}/public/images/soon.jpg" class="img-responsive" style="height: 292px;margin-top: -22px;width: 100%;">
            </div>
        </div>
                </div>
            </div>
...
<br>
    ...
    <div class="modal-dialog diolog-css" role="document" >
        ...
        <!--/.Content-->
                ...
                <div class="input-group m-t-1" style=" position: absolute;
    top: 320px;
    width: 90%;">
                    <input id="btn-input" onFocus={this._slideViewUpToShowInputElement(refName)} type="text" class="form-control message input-sm" placeholder="Type your message here..." style=";margin:0;padding: 0px 0px;">
            ...
            <div class="col-md-7" style="display:none; margin-top: 40px; margin-bottom: 40px" id="incident-detail-id">
                <h3 class="h3">INCIDENT DETAIL</h3>
                <div class="">
                    <div class="p-3 location" style="padding: 10px !important;height: auto;">
                        <label class="form-label">Is this incident taking place now?</label>
                        <input class="form-control" type="text" name="name" id="lp-name" value="{{$video->lost_child['incident_time']}}" readonly><br/>

                        <label class="form-label">Are you in immediate danger?</label>
                        <input class="form-control" type="text" name="gender" id="lp-gender"  value="{{$video->lost_child['incident_date'] }}" readonly><br/>

                        <label class="form-label">Please indicate the severity level of this incident?</label>
                        <input class="form-control" type="text" name="age" id="lp-age"  value="{{$video->lost_child['incident_nature']}}" readonly><br/>
                   </div>
                </div>
            </div>
...
<script>
    ...
    var Ref = firebase.database().ref('/{{$video->video_id}}');
    console.log('firebase called');

    Ref.on("child_changed", function(snapshot) {
        console.log('chiled trigered');
        if(snapshot.key == 'hasUpdatedMessage' && snapshot.val() == true){
            console.log('Method called');
            $.ajax({
                url: "{{url('/')}}/get_video_information/{{$video->video_id}}",
                type: 'get',
                cache: "false",
                contentType: false,
                processData: false,
                success: function(data) {
                    var div = document.getElementById("incident-detail-id");
                    div.style.display = "block";
                    setTimeout(function(){
                        $('#lp-name').val(data.lost_child.incident_time);
                        $('#lp-gender').val(data.lost_child.incident_date);
                        $('#lp-age').val(data.lost_child.incident_nature);
    ...
    function initMap() {
        ...
        var marker1 = new google.maps.Marker({
            position: {lat: parseFloat(<?php echo $lat; ?>), lng: parseFloat(<?php echo $long; ?>)},
            map: map,
            title: 'Hello World!',
        ...
        var flightPlanCoordinates = [
            ...
            {lat: parseFloat(<?php echo $location->latitude; ?>), lng: parseFloat(<?php echo $location->longitude; ?>)},
            <?php }?>
        ];
    ...
    var intervalId = setInterval(function () {
        ...
        $.ajax({
            type : 'get',
            url : '{{ url('contact_center/ajax/getLatlongs/'.$video->video_id) }}',
            success : function(data){
                $.each(data,function(key,val){

                flightPlanCoordinates.push({lat: val.latitude, lng: val.longitude });
                });
        var flightPath = new google.maps.Polyline({
            path: flightPlanCoordinates,
            geodesic: true,
...
<script type="text/javascript">
    ...
    var api_key = '{{{$opentok_api}}}';
    var session_key = '{{{$video->session_token}}}';
    var token = '{{{$video->opentok_token}}}';
...
*/
    ...
    session.on("streamDestroyed", function(event) {
        ...
        setInterval(function () {
            $.ajax({
                url: "{{ url('/') }}/checkArchiveId/{{$video->archive_id}}",
                method: "get",
                success: function(data) {
                    if(data=="success"){
                        location.reload();
...
//                request.send(formData);
   ...
   /* $('#btn-input').on('focus', function() {
    document.body.scrollTop = $(this).offset().top + 100;
});*/
