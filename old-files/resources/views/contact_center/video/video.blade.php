...
if ($values == 'report_tip') {
	...
	$videos = \App\Videos::with('user')->where('archive_id', '!=', '')->where('type', '=', 'anonymous')->whereIn('user_id', $user_ids)->whereIn('video_id', $video_ids)->orderBy('id', 'desc')->paginate(20);
} else {
	$url = 'https://ifollow-cc-3f29a.firebaseio.com/.json?orderBy="isActive"&equalTo=false';
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $url);
	...
	$video_ids = array_keys((array) $keys);
	$videos = \App\Videos::with('user')->where('archive_id', '!=', '')->where('type', '=', 'anonymous')->whereIn('user_id', $user_ids)->whereIn('video_id', $video_ids)->paginate(20);
