...
if (Session::has('monitor_admin')) {
	...
	$user_ids = \App\Users::whereIn('organization_id', $organization_ids)->pluck('user_id')->all();
	$values = basename(parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH));
	if ($values == 'safewalk') {
		$url = 'https://ifollow-realtorsafewalk.firebaseio.com/.json?orderBy="isActive"&equalTo=true';
		$ch = curl_init();
		...
		$videos = \App\Videos::with('user.user_emergency_contacts', 'user.organization')->where('archive_id', '!=', '')->where('type', '=', 'panic')->whereIn('user_id', $user_ids)->whereNotIn('video_id', $video_ids)->orderBy('id', 'desc')->get();
		...
		$keys = json_decode($keys);
		$video_ids = array_keys((array) $keys);
		$videos = \App\Videos::with('user', 'user.organization')->where('archive_id', '!=', '')->where('type', '=', 'panic')->whereIn('user_id', $user_ids)->whereIn('video_id', $video_ids)->orderBy('id', 'desc')->get();
...
@section('title')
    {{ config('app.name') }} | @if($values === 'pending_safewalk') Active Panics @else Ended Panics @endif
@stop
@section('css')
                ...
                <div class="page-content-wrapper">
                    ...
                    <div class="page-content">
                       ...
                       {{--  <h1 class="page-title"> {{ session('contact_center_admin.0.name') }}
                            <small>@if(Request::url()  == url('/pending_safewalk')) Open Panic @else Completed Panic @endif</small>
                        </h1> --}}
                        <!-- END PAGE TITLE-->
                                ...
                                <div class="portlet light bg-inverse">
                                    <div class="portlet-title">
                                        <div class="caption font-green-sharp">
                                            ...
                                            <span class="caption-subject"> @if($values === 'pending_panics') Active Panic Alert @else Ended Panic Alert @endif</span>
                                                ...
                                                <tbody>
                                                     @foreach($videos as $video)
                                                        <tr>
                                                            ...
                                                            @else
                                                                <td ><span style="width: 90px; height: 16px;    margin-top: 3px;" class="btn green">
                                                                         Ended
                                                                     </span>
                                                                </td>
                                                            @endif
                                                            ...
                                                            <td>{{ $video->user->organization->organization_name }}</td>
                                                            <td>{{ $video->user->organization->time_zone->timezone_code }}</td>
                                                            ...
                                                            <td>
                                                              ...
                                                              <td><button tabindex="0" role="button" data-trigger="focus" type="button" class="btn btn-fb panic-note btn-fb" data-toggle="popover" onclick="showNote(this)"  data-placement="top" title="{{ $video->user->first_name }} Notes" data-content="{{ $video->notes?$video->notes:"N/A" }}" style="width: 90px;height: 16px; margin-top: 3px; ">View Notes</button></td>
                                                            <td>
                                                                <div class="btn-group">
                                                                    <button class="btn btn-fb dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="width: 50px;height: 16px; margin-top: 6px; ">Actions</button>
...
function showNote(current)
{
    ...
    $(current).popover('toggle')
        ...
        bootbox.confirm("Are you sure do you want to delete safewalk?", function (result) {
            if (result == true) {
                  $.ajax({
                "method" : "POST",
