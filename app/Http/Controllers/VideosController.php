...
class VideosController extends Controller {
	...
	public function create_image_session(Request $request) {
		...
		if (apiHelper::authenticate($user_id, $auth_id)) {
			...
			$video->video_id = $video_id;
			$video->user_id = $user_id;
			if ($request->has('lost_child')) {

				$video->lost_child = $request->lost_child;
			}
	...
	public function sendMessageToContacts($user_id, $video_id) {
		...
		try {
			Mail::send([], [], function ($message) use ($contacts_email, $type, $video_id) {
				...
				} elseif ($type == 'lost_child') {
					$body = 'A new incident report is in progress. View Here: https://controlhub.ifollow.com/dashboard?type=' . $type . '&video_id=' . $video_id;
				} else {
	...
	public function showVideoSafe($video_id) {
		...
		if ($video->archive_id) {
			...
			if ($httpCode == 405 or $httpCode == 404) {
				if ($video->is_image) {
					return view('contact_center.video.show_image', compact('opentok_api', 'video', 'user'));
				} else {
					return view('contact_center.video.show_video_safe', compact('opentok_api', 'video', 'user'));

				}
			} else {
				return view('contact_center.video.show_video_safe_complete', compact('opentok_api', 'video', 'archive_url', 'user'));
			}
			// return view('videos.show_video_complete',compact('opentok_api','video','archive_url','user'));
		} else {
			return view('contact_center.video.show_video_safe', compact('opentok_api', 'video', 'user'));
	...
	public function update_lost_child(Request $request) {
		$user_id = $request->header('user-id');
		$auth_id = $request->header('auth-id');
		if (apiHelper::authenticate($user_id, $auth_id)) {
			$rules = array('video_id' => 'required', 'lost_child' => 'required');
			$validations = Validator::make($request->all(), $rules);
			if ($validations->fails()) {
				$messages = $validations->messages();
				$data = apiHelper::validation_error('Validation errors', $messages);
	...
	public function getLatlongs($video_id) {
		$locations = \App\VideoLocationTracking::where('video_id', $video_id)->get();
		return response()->json($locations);
	}

	public function get_video_information($video_id) {
		$data = Videos::where('video_id', $video_id)->first();
		return $data;
	}
