...
<script type="text/javascript" src="<?php echo url('/') ?>/public/js/material.js"></script>
        ...
        $('#monitor_form').validate({
            ...
            submitHandler:function(form){
                ...
                $('#logInMonitorButton').attr('disabled',true);
                $('#logInMonitorButton').html('Loading ...');
                $.ajax({
                    url:"{{ url('/') }}/monitor-hub/ajax/login",
                    type:'post',
                    ...
                    {
                        $('#logInMonitorButton').attr('disabled',false);
                        $('#logInMonitorButton').html('Log In');
                        if(data=='login successful')
                        {
                            toastr["success"](data);
                            @if(empty(Request::get('returnUrl')))
                            url='{{ url('/') }}/monitor-hub/all-panics';
                            @elseif(!empty(Request::get('returnUrl')))
                            url= '<?php echo htmlspecialchars_decode(Request::get('returnUrl')); ?>';
                            @endif
        ...
        $('#monitor_forget_form').validate({
            ...
            submitHandler:function(form){
                ...
                $('#forgetMonitorButton').attr('disabled',true);
                $('#forgetMonitorButton').html('Loading ...');
                $.ajax({
                    url:"{{ url('/') }}/monitor-hub/ajax/forget",
                    type:'post',
                    data: $("#monitor_forget_form").serialize(),
                    error:function(){
                        url='{{ url('/') }}';
                    },
                    success:function(data)
                    {
                         url='{{ url('/') }}/monitor-hub';
                        $('#forgetMonitorButton').attr('disabled',false);
                        $('#forgetMonitorButton').html('Verify');
