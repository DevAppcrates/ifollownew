...
class ContactCenterController extends Controller {
	...
	public function invitees() {
		...
		$invitees = Invitees::with('user_tags.tag')->where('organization_id', $organization_id)->orderBy('created_at', 'desc')->get();
	...
	public function notification(Request $request) {
		...
		if (Session::has('monitor_admin')) {
			$timeZoneCode = session('monitor_admin.0.organizations.0.organization.time_zone.timezone_code');
		} elseif (Session::has('contact_center_admin')) {
			$timeZoneCode = session('contact_center_admin.0.time_zone.timezone_code');
		}

		if ($request->date && $request->time && $request->am_pm) {
			config(['app.timezone' => $timeZoneCode]);
		...
		$valid = Validator::make($request->all(), ['time' => ['required_if:schedule,1', new PublishDateValidate($timestamp, $timeZoneCode, $request->schedule)], 'date' => 'required_if:schedule,1', 'am_pm' => 'required_if:schedule,1'], ['time.required_if' => 'time is required to schedule the Mass Notification properly', 'date.required_if' => 'date is required to schedule the Mass Notification properly', 'am_pm.required_if' => 'AM/PM must be set']);
		if ($valid->fails()) {
		...
		if (Session::has('monitor_admin')) {
			...
			$organization_id = $user[0]['organizations'][0]['organization']['organization_id'];
			$admin_id = $user[0]['organizations'][0]['id'];
			$organization_name = $user[0]['organizations'][0]['organization']['organization_name'];
			$name = $user[0]['organizations'][0]['organization']['name'];
		} elseif (Session::has('contact_center_admin')) {
			$user = Session::get('contact_center_admin');
			$organization_id = $user[0]['organization_id'];
			$admin_id = $user[0]['id'];
		...
		$current_time = new DateTime('now', new DateTimeZone($timeZoneCode));
				...
				foreach ($users as $user) {
					try {
						Mail::send([], [], function ($message) use ($user, $notification, $organization_name) {
							...
							$message->to($user->email)->subject($notification->priority . $notification->title . ' - ' . $organization_name)->setBody($body, 'text/html');
					...
					$body = $notification->priority . ' ' . $request->title . ' - ' . $organization_name . "\n\n" . $notification->title . "\n\n";
					if ($notification->notification) {
			...
			if ($request->hasFile('csv')) {
				...
				foreach ($results as $result) {
					if (!Invitees::where('organization_id', $organization_id)->where('email', $result['email'])->first()) {
						...
						$data['code'] = $user[0]['code'];
						$data['email'] = $user[0]['email'];
						$name = $result['name'];
						$email = $result['email'];
						$subject = $request->subject;

						Mail::send('mail.invite', ["data" => $data], function ($message) use ($name, $email, $subject, $organization_name) {
							$message->from('alert@ifollow.com', $organization_name);
	...
	public function edit_my_schedule(Request $request) {
		...
		if (Session::has('monitor_admin')) {
			$adminId = session('monitor_admin.0.id');
			$organizationId = session('monitor_admin.0.organizations.0.organization_id');
		} elseif (Session::has('contact_center_admin')) {
			$adminId = session('contact_center_admin.0.id');
			$organizationId = session('contact_center_admin.0.organization_id');
		}
		...
		foreach ($days as $day) {
			...
			if ($request->get($day_name . '_status') == 'active'):
				$schedule = Schedule::updateOrCreate([
					...
					[
						'open_time' => $request->get($day_name . '_start_time'),
						'open_time_format' => $request->get($day_name . '_start_time_am_pm'),
						'close_time' => $request->get($day_name . '_close_time'),
						'close_time_format' => $request->get($day_name . '_close_time_am_pm'),
						'status' => $request->get($day_name . '_status'),
					]);

				echo $request->get($day_name . '_start_time') . ' = > if <br>';
				var_dump($schedule);
			else:
				$schedule = Schedule::updateOrCreate(['admin_id' => $adminId, 'organization_id' => $organizationId, 'day_id' => $day['id']], ['status' => $request->get($day_name . '_status')]);
			endif;
		}
	}
	...
	public function contact_center_forget(Request $request) {
		...
		Mail::send([], [], function ($message) use ($organization, $token) {
			...
			$body .= "<p>
                       We Have received your request to reset the password for your iFollow Control-hub account. If you did-not request this change, please contact your iFollow rep immediately. Otherwise: Please click the following link to securely reset your password.
                       </p>";
	...
	public function notification_user_status($user_id, $notification_id, $type) {
		...
		return Redirect::to('http://confirms.ifollow.com/');
